...
```env
DATABASE_URL=postgresql+psycopg://postgres:postgres@localhost:5432/civisense
GROQ_API_KEY=your_groq_key   # set this when available
```

3. Run database migrations (if using Alembic) and start the server:
...
- Upload: `POST /api/v1/documents/upload` (multipart file)
- List: `GET /api/v1/documents`
- Get: `GET /api/v1/documents/{document_id}`
- Delete: `DELETE /api/v1/documents/{document_id}`
- Start analysis: `POST /api/v1/analysis/{document_id}`
- Get analysis: `GET /api/v1/analysis/{document_id}`

## Groq / LangChain notes

The backend will attempt to use Groq via LangChain when `GROQ_API_KEY` is set. If the key is missing the project falls back to a deterministic local LLM placeholder (see `app/services/llm_service.py`) so the pipeline remains testable without external credentials. Replace the placeholder logic with production prompts and Groq calls once your key and prompt templates are ready.

## Testing

This repository does not include a full test suite yet. Recommended next steps:

- Add unit tests using pytest and an in-memory SQLite database for DB-related tests.
- Mock LLM/Groq calls so tests don't require network access.
- Add an integration test that uploads a small sample PDF, runs analysis, and asserts expected DB records and API responses.

If you want, I can add a small pytest suite (mocking Groq) to cover upload + analysis flow next.
